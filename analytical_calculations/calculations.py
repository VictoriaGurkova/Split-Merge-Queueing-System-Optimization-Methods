...
class Calculations:
    ...
    def calculate_performance_measures(self, distribution: list, states: list) -> None:
        ...
        for state, state_probability in enumerate(distribution):
            ...
            self.calculate_avg_free_servers_if_queues_not_empty(
        ...
        self.performance_measures.failure_probability = (
            p1 * self.performance_measures.failure_probability1
    ...
    def calculate_response_time(self) -> None:
        effective_lambda1 = self.params.lambda1 * (
            1 - self.performance_measures.failure_probability1
        )
        effective_lambda2 = self.params.lambda2 * (
            1 - self.performance_measures.failure_probability2
        )
        ...
        self.performance_measures.response_time = (
            self.performance_measures.avg_queue1
    ...
    def calculate_response_time_solution1(
        self, queue_waiting1: float, queue_waiting2: float
    ) -> None:
        self.performance_measures.response_time1 = (
            queue_waiting1
            + harmonic_sum(self.params.fragments_numbers[0]) / self.params.mu
        )
        self.performance_measures.response_time2 = (
            queue_waiting2
            + harmonic_sum(self.params.fragments_numbers[1]) / self.params.mu
        )

    def calculate_response_time_solution2(
        self, effective_lambda1: float, effective_lambda2: float
    ) -> None:
        self.performance_measures.response_time1 = (
            self.performance_measures.avg_queue1
            + self.performance_measures.avg_demands_on_servers1
    ...
    def calculate_avg_queue(
        self, states: list, state: int, state_probability: float
    ) -> None:
    ...
    def calculate_avg_free_servers(
        self, states: list, state: int, state_probability: float
    ) -> None:
        self.performance_measures.avg_free_servers += (
            ...
            - (
                ...
                + len(states[state][1][1]) * self.params.fragments_numbers[1]
            )
        ) * state_probability

    def calculate_avg_free_servers_if_queues_not_empty(
        self, states: list, state: int, state_probability: float
    ) -> None:
    ...
    def calculate_avg_demands_on_servers(
        self, states: list, state: int, state_probability: float
    ) -> None:
        self.performance_measures.avg_demands_on_servers1 += (
            len(states[state][1][0])
            * self.params.fragments_numbers[0]
            * state_probability
        ) / self.params.fragments_numbers[0]

        self.performance_measures.avg_demands_on_servers2 += (
            len(states[state][1][1])
    ...
    def calculate_failure_probability(
        ...
        if (states[state][0][0] == self.params.queues_capacities[0]) and (
            ...
            self.performance_measures.blocked_all_queues_probability += (
                state_probability
            )

    def get_norm_const(self) -> float:
        class1_probability = self.params.lambda1 / (
            self.params.lambda1 + self.params.lambda2
        )
        class2_probability = 1 - class1_probability
        queue_waiting_probability1 = class1_probability * (
            1 - self.performance_measures.failure_probability1
        )
        queue_waiting_probability2 = class2_probability * (
            1 - self.performance_measures.failure_probability2
        )
