    ...
    for i in range(x + 1):
        for j in range(y + 1):
            total_number_of_tasks = (
                params.fragments_numbers[0] * i + params.fragments_numbers[1] * j
            )
...
def get_all_state_with_queues(
    server_states: list, queues_capacities: list, params: Params
) -> list:
    states = []
    queue_states = set(
        itertools.product(
            range(queues_capacities[0] + 1), range(queues_capacities[1] + 1)
        )
    )
...
def check_possible_state(q_state: tuple, state: list, params: Params) -> bool:
    free_servers_number = get_free_servers_number_for_server_state(params, state)
...
def get_free_servers_number_for_server_state(params: Params, server_state: list) -> int:
    ...
    if number < 0:
        raise Exception(
            "Number of free servers for states < 0, " "it is not correct state"
        )
    return number
