...
from optimization.iterative_method.reward import (
    get_rewards_for_states,
    get_income_matrix,
)
...
class IterativeMethod:
    ...
    def run(
        ...
        for iter in range(max_iter):
            ...
            if print_iteration_results:
                ...
                policy = Policy(
                    stationary_strategy, self.states_with_policy, self.params
                )
    ...
    def apply(self, print_iteration_results=True):
        ...
        g, stationary_strategy = self.run(
            p,
            q,
            state_indices,
            self.all_states_num,
            self.states_with_policy_num,
            100000,
            print_iteration_results,
        )
        ...
        for index, state in enumerate(self.states_with_policy):
            if stationary_strategy[index] == 0:
                print(
                    "After leaving the state", state, "take a demand from the 1st queue"
                )
            elif stationary_strategy[index] == 1:
                print(
                    "After leaving the state", state, "take a demand from the 2st queue"
                )
    ...
    def get_q(self):
        ...
        rewards = get_rewards_for_states(
            self.all_states, self._reward_function_for_states
        )
