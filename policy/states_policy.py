...
class Policy:
    def __init__(self, policy_vector: tuple, states_with_policy: list, params: Params):
        ...
        self.adjacent_states: dict = {
            state: self.define_adjacent_states(state) for state in states_with_policy
        }
    ...
    def define_adjacent_states(self, state: tuple) -> list:
        ...
        for server_state in state[1]:
            if last_fragment in server_state and have_a_choice(
                state, index, self.params
            ):
                # generate state when first class is taken
                adjacent_states.append(
                    self.get_state_after_leaving(state[0], state[1], index, 0)
                )
                # generate state when second class is taken
                adjacent_states.append(
                    self.get_state_after_leaving(state[0], state[1], index, 1)
                )
                index = 0
                break
            index += 1
    ...
    def get_state_after_leaving(
        self, queues_state, server_state, class_id_leave, class_id_take
    ) -> tuple:
        if class_id_leave == 0:
            if class_id_take == 0:
                # TODO: export in def
                possible_demands_number_for_taking = (
                    get_free_servers_after_leaving(server_state, 0, self.params)
                    // self.params.fragments_numbers[0]
                )
                available_demands_number_for_taking = (
                    queues_state[0]
                    if queues_state[0] <= possible_demands_number_for_taking
                ...
                return tuple(
                    (
                        (
                            queues_state[0] - available_demands_number_for_taking,
                            queues_state[1],
                        ),
                ...
                available_demands_number_for_taking = (
                    queues_state[1]
                    if queues_state[1] <= possible_demands_number_for_taking
                    else possible_demands_number_for_taking
                )
                taken_demands = (
                    self.params.fragments_numbers[1],
                ...
                return tuple(
                    (
                        (
                            queues_state[0],
                            queues_state[1] - available_demands_number_for_taking,
                        ),
            ...
            if class_id_take == 0:
                ...
                possible_demands_number_for_taking = (
                    get_free_servers_after_leaving(server_state, 1, self.params)
                    // self.params.fragments_numbers[0]
                )
                available_demands_number_for_taking = (
                    queues_state[0]
...
def get_policed_states(states: list, params: Params) -> list:
    ...
    class_index_with_max_fragments_number = params.fragments_numbers.index(
        max_fragments_number
    )
    ...
    for state in states:
        ...
        last_fragment_of_any_demand = (
            last_fragment in state[1][0] or last_fragment in state[1][1]
        )
        if all_queues_not_empty and last_fragment_of_any_demand:
            ...
            # policy is applied if a class with fewer fragments number leaves and there are free services
                ...
                if (
                    free_servers_number + leaving_fragments_number
                    >= max_fragments_number
                ):
                    policed_states.append(state)
